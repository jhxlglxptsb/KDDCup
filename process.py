    ...
    if type == 0:
        ...
        for i, x in enumerate(aq):
            for j, feat in enumerate(features):
                bj_aq[i, j] = aq[i][feat]
        np.save(open('./files/bj_aq_'+station+'.npy', 'wb'), bj_aq)
    else:
        ld_aq = np.zeros((len(aq), 3))
        for i, x in enumerate(aq):
            for j, feat in enumerate(features):
                ld_aq[i, j] = aq[i][feat]
        np.save(open('./files/ld_aq_'+station+'.npy', 'wb'), ld_aq)
...
aq_all = json.load(open('beijing_aq.json'))
print('> get %d entries' % len(aq_all))
aq_london = json.load(open('london_aq.json'))
